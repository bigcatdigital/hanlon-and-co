...
var bcFunctions = function bcAppJS() {
  ...
  **/
  debug = true;
  var theBody = document.querySelector('html');
  var surveyTimeout = 10 * 1000;
  if (debug) {
    surveyTimeout = 5 * 1000;
    console.log("[debug]: ".concat(surveyTimeout));
  }
  window.setTimeout(function () {
    debug = true;
    if (debug) {
  ...
  function doSiteSurvey() {
    ...
    if (takeSurvey) {
      takeSurvey.addEventListener('click', function (evt) {
        ...
        bcSetCookie('site-survey-response', 'taken', {
          'expires': expiry
        });
        if (debug) {
    ...
    if (rejectSurvey) {
      rejectSurvey.addEventListener('click', function (evt) {
        ...
        var expiry = new Date();
        expiry.setMonth(expiry.getMonth() + 1);
        bcSetCookie('site-survey-response', 'rejected', {
          'expires': expiry
        });
        if (debug) {
          console.log("Debug:: site-survey-response cookie: ".concat(bcGetCookie('site-survey-response')));
  ...
  function showSurveyModal() {
    var $surveyBlock = document.querySelector('#bc-site-survey');
    if (debug) {
      console.log("Debug:: showSurveyModal() context");
    }
    if ($surveyBlock && $surveyBlock !== undefined) {
      theBody.classList.add('bc-survey-modal-visible');
      $surveyBlock.classList.add('bc-is-active');
    }
  }
  function hideSurveyModal() {
    ...
    if ($surveyBlock && $surveyBlock !== undefined) {
      ...
      $surveyBlock.classList.remove('bc-is-active');
    }
  }
